...
__all__ = ['construct_df_csv', 'drop_columns', 'update_type_date', 'rename_columns', 'add_column_up', 'df_multiindex',
           'compute_average_A_B', 'compute_rotations', '__deformee__', 'column_integration', 'column_deformee_relative',
           'run_computation_inklino', 'extract_df_datetube', 'build_fig_defo']
...
# Cell
def column_deformee_relative(df):
  """
  Ajoute la colonne des déformations relatives : la valeur de référence est la 1ère mesure
  """
  df = df.sort_index(axis=0)
  # Pour s'assurer que les hauteurs coincident avec les valeurs de defo ajoutées
...
def run_computation_inklino(inklino_file_name):
  ...
  df_inklino = column_deformee_relative(df_inklino)
